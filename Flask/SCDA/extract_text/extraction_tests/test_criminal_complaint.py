...
class test_criminal_complaints(unittest.TestCase):
    doc = open(os.path.abspath("test_textdumps/CC.txt")).read()
    ...
    def test_codes(self):
        input = extract_fields.find_codes(self.doc) 
        expected = str(['90/23/E', '90/23/H', '90/23/G', '90/34J', '666666'])
        self.assertEqual(input, expected)
